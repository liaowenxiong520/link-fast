...
public class ProxyRegionIT {
    ...
    void syncRegionTreeToDb_fullTree_realApiAndPersist() throws Exception {
        ...
        Long beforeCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM proxy_region", Long.class);
        assertNotNull(beforeCount);

        // 返回的是总处理数（插入+更新）
        int batchModifiedCount = proxyRegionService.syncRegionTreeToDb(null);
        Long dbCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM proxy_region", Long.class);
        assertNotNull(dbCount);
        assertTrue(dbCount > 0,"同步完成后Region表的总记录数不应该等于0");

        System.out.println("[ProxyRegionIT] 实际处理的地域记录数: " + batchModifiedCount);
        System.out.println("[ProxyRegionIT] 同步前 region 表总行数: " + beforeCount);
        System.out.println("[ProxyRegionIT] 当前 region 表总行数: " + dbCount);
    }
    ...
    void syncRegionTreeToDb_byCodes_realApiAndPersist() throws Exception {
        ...
        System.out.println("[ProxyRegionIT] codes=" + codes + " 同步节点数: " + synced);
        System.out.println("[ProxyRegionIT] 当前 region 表总行数: " + dbCount);
    }
}
